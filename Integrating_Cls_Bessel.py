...
def hypfnearone(ell,nu,t):
    ...
    # print(hypf((2*ell+nu)/4,(-2*ell+nu-2)/4,nu/2,ttilde))
    # print(hypf((2*ell-nu+4)/4,(-2*ell-nu+2)/4,2-nu/2,ttilde))
    # print(t**(-ell-nu/2))
...
def Dl_wg(chi,ell,FIELD,experiment):
    ...
    #interp_dlwg_first is be -d^2d/chi^2 +2/chi d/dchi. double check this.
...
def intWgalaxy_lowell(ells,nus,ts,chiresolution,SPECTRUM,experiment,test=False):
    if np.max(ells>11):
...
def intWgalaxy_highell(ell,nus,tresolution,chiresolution,SPECTRUM,experiment,test=False):
    ...
    for nu_index,nu in enumerate(nus):
        #it is too much for my compute to handle doing this as an array because nu x tresolution x chiresolution is crazy big
        mint=mints[nu_index]
...
def intIW(ells,nus,tresolution,chiresolution,SPECTRUM,experiment,test=False):
  ...
  #  print(fact2lessthan10[0:3])
    ...
    for ell_index, ell in enumerate(ells): 
        ...
        if ell>10:
            ...
            fact1=intWgalaxy_highell(ell,nus,tresolution,chiresolution,SPECTRUM,experiment,test) 
               #this is nu x tres
            ...
            for nu_index, nu in enumerate(nus):
                ...
                if(nu_index%10==0):
                    ...
                    plt.subplot(121)
                    plt.plot(ts,integrand[nu_index,:].real,'o')
                    plt.subplot(122)
                    plt.plot(ts,integrand[nu_index,:].imag,'o')
                    print(ell,nu)
                    plt.show()
                    plt.figure(figsize=(15,5))
                    plt.subplot(121)
                    plt.plot(ts,fact1[nu_index,:].real,'o')
                    plt.subplot(122)
                    plt.plot(ts,fact1[nu_index,:].imag,'o')
                    print(ell,nu)
                    plt.show()
                    plt.figure(figsize=(15,5))
                    plt.subplot(121)
                    plt.plot(ts,fact2[nu_index,:].real,'o')
                    plt.subplot(122)
...
def to_transform2(k):
    ...
    if test:
        return 1/k*Pk(k,True,True)*np.exp(-((k)/10)**2)
   # print(test)
    return k**3*Pk(k)*np.exp(-((k)/10)**2)#Pk(k,True,True)*np.exp(-((k)/10)**2)
...
def _Cls_Exact_Wrapper(FIELD,ells,tresolution,chiresolution,maxfreq,number_of_clustering_bins,bias,experiment=None,test=False):
    ...
    if FIELD[0]=="shear":
        ...
        for i in range(0,number_of_clustering_bins):
            #COME BACK TO THIS
                   print("getting bin",i)
                   SPECTRUM=[FIELD,density_fields[i]]
...
def Cls_Exact(ells,SPECTRUM,tresolution,chiresolution,maxfreq,experiment,bias,test=False):
            ...
            #    plt.plot(chis,integrand)
             #   plt.show()
              #  print([[chis[i],(Pk((ell+1/2)/chis)[i],True,True)] for i in range(0,len(chis))])
                #plt.plot(chis,Pk((ell+1/2)/chis,True,True))
               # plt.show()
