...
def dl_wg_ell_independent(chi,FIELD,number_of_clustering_bins):
...
interp_dlwg_clustering=interp1d(chis,dl_wg_ell_independent(chis,["density",0,"magic"],number_of_clustering_bins))
interp_dlwg_shears=[interp1d(chis,dl_wg_ell_independent(chis,["shear",i,"magic"],number_of_clustering_bins))for i in range(0,10)]
...
def intWgalaxy_lowell(ells,nus,ts,chiresolution,SPECTRUM,experiment):
    if np.max(ells>11):
...
def intWgalaxy_highell(ell,nus,tresolution,chiresolution,SPECTRUM,experiment):
...
def intIW(ells,nus,tresolution,chiresolution,SPECTRUM,experiment):
    ...
    fact1=intWgalaxy_lowell(ells[ells<11],nus,ts,chiresolution,SPECTRUM,experiment)#ellxtxnushaped array
    ...
    for ell_index, ell in enumerate(ells): 
        ...
        if ell>10:
            ...
            fact1=intWgalaxy_highell(ell,nus,tresolution,chiresolution,SPECTRUM,experiment) 
               #this is nu x tres
